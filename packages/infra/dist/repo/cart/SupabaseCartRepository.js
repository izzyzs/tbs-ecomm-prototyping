...
export class SupabaseCartRepository {
    ...
    async retrieveSingleCartItem(cartId, productId) {
        ...
        console.log("price from db: ", skuData.price);
        const price = InventoryMapper.priceDBtoDomain(skuData.price);
        console.log("created price: ", price);
        const cartItem = new CartItem(new CartItemId(cartItemData.id), new ProductId(requiredField(cartItemData.product_id, SupabaseError, "cartItemData.product_id")), new SKU((skuData.upc ?? skuData.manufact_sku ?? skuData.custom_sku)), // one of these three values will undoubtedly be set.
        requiredField(skuData.item, SupabaseError, "skuData.item"), requiredField(skuData.brand, SupabaseError, "skuData.brand"), price, new Quantity(requiredField(cartItemData.quantity, SupabaseError, "cartItemData.quantity")));
        return cartItem;
    }
