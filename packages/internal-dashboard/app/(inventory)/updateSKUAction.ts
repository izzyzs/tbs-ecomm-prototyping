...
export const updateSKUAction = async (prevState: ToastResponse | null, formData: FormData): Promise<ToastResponse> => {
    ...
    const { data: initial, error: readErr } = await supabase.from("inventory").select("publish_to_ecom, qty, item, price_in_pennies, tax, default_cost").eq("id", id).single();
    ...
    const current = {
        ...
        price_in_pennies: getNumber(formData, `price_in_pennies`),
    ...
    const { error: updateErr } = await supabase.from("inventory").update(patch).eq("id", id).select("publish_to_ecom, qty, item, price_in_pennies, tax, default_cost").single();
