...
import {CartGateway, DefaultCartGateway, MaxQuantityError, SupabaseInventoryRepository} from "@tbs/infra";
...
export class AddOrIncrementCartItem {
    ...
    async execute(productId: ProductId, owner: CartOwner): Promise<CartItem> {
        ...
        const limit = await this.inventoryRepository.getMaxPurchaseQuantity(cartItemDraft.productId);

        if (cartItemDraft.quantity.amount == limit) {
            throw new MaxQuantityError(`You have reached the max quantity: ${limit}.`);
        }
