...
type LineItem = { price_data: PriceData; quantity: number; tax_rates: string[] };

const cartItemStateToLineItemMapper = (cartItem: CartItemState): LineItem => {
    return {
        price_data: {
            currency: "usd",
            product_data: {
        ...
        tax_rates: [process.env.NYC_SALES_TAX!],
    };
};
...
export async function POST(request: NextRequest) {
    try {
        ...
        const lineItems: LineItem[] = cartItems.map((cartItem: CartItemState) => cartItemStateToLineItemMapper(cartItem));
        ...
        const status = typeof err === "object" && err != null && "statusCode" in err && typeof (err as any) === "object" && (err as any).statusCode === "number" ? (err as any).statusCode : 500;
